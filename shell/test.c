...
typedef struct {
    child_t child_list[NUM]; /* プロセス管理リスト */
...
void write_new(pid_t pid, bool is_background, process_t *process_manager) {
    ...
    */
    ...
    while (1) {
        ...
        if (pid > 0) {
            ...
            //プロセスマネージャに新しいプロセスを書き加える
            write_new(pid, is_background, &process_manager);
